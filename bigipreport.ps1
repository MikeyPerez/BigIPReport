...
Add-Type @'
	...
	public class VirtualServer
	{
		...
		public string clientsslprofile;
        public string serversslprofile;
...
function Cache-LTMInformation {
	...
	for($i=0;$i -lt ($VirtualServers.Count);$i++){
		...
		$ObjTempVirtualServer.clientsslprofile = "None";

		$VirtualServerProfiles[$i] | ForEach-Object {
			if([string]($_.profile_type) -eq "PROFILE_TYPE_CLIENT_SSL"){
				$ObjTempVirtualServer.clientsslprofile = $_.profile_name;
			}
		}

        $ObjTempVirtualServer.serversslprofile = "None";

		$VirtualServerProfiles[$i] | ForEach-Object {
			if([string]($_.profile_type) -eq "PROFILE_TYPE_SERVER_SSL"){
				$ObjTempVirtualServer.serversslprofile = $_.profile_name;
			}
		}
...
'@)
	...
	[void]$Global:HTML.AppendLine(@'
				<th class="sslProfileHeaderCell"><input type="text" name="clientsslProfile" size=6 value="SSL" class="search_init" data-column-name="SSL Profile" data-setting-name="showSSLProfileColumn"/></th>
...
"@)
		...
		[void]$Global:HTML.AppendLine(@"
						<td class="centeredCell">
							$(
								If ($ObjVirtualServer.clientsslprofile -ne "None") {
									"Yes"
								} Else {
									"No"
								}
